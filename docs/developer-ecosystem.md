...
| Hugging Face | Inference Providers' Responses API accepts a remote MCP `server_url` and `allowed_tools` | [`examples/integrations/huggingface`](../examples/integrations/huggingface) | Supported for inference. The Hub dataset card and metadata are prepared, but no public Hub dataset URL is recorded. |
...
- A Hugging Face account owner must create the dataset repository, review the
  dataset license treatment, upload the dated CSV and prepared card, then
  record and verify the canonical Hub URL. Until then the dataset status is
